...
public class Navigation : Singleton<Navigation>
{
    ...
    public void FinishTeleport(Ray ray)
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
        {
            ...
            StartCoroutine(QuantumTeleport(transform.position, hit.point + new Vector3(transform.position.x - transform.GetChild(2).position.x, 0f, transform.position.z - transform.GetChild(2).position.z)));
            feedback.SetActive(false);
        }
        else
        {
